 ...
 * Flips an application's status to 'approved'.
 * Allowed for `admin` and `membership_staff`.
 */
export async function POST(
  ...
  // RLS should mirror this check so Supabase remains the source of truth.
  let query = supabase
    ...
    .eq('id', id)
    .eq('status', 'pending')   // idempotency: only flip from pending
